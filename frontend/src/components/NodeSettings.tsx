...
const NodeSettings = () => {
	...
	const handleShowInputHandleToggle = (
		value: boolean,
		input: InputDataItem
	) => {
	...
	const handleShowOutputHandleToggle = (
		value: boolean,
		output: OutputDataItem
	) => {
		const node = getNodeById(settingNodeID!);
		if (!node) return;
		setNode(settingNodeID!, {
	...
	const handleShowOutputDataToggle = (
		value: boolean,
		output: OutputDataItem
	) => {
		const node = getNodeById(settingNodeID!);
	...
	return (
		<div className='p-5 flex flex-col overflow-auto h-full'>
			...
			<div className='overflow-auto flex-1'>
				{getNodeById(settingNodeID!)?.data.inputs && (
					<div className='rounded border border-1 p-3 border-blue-500'>
				...
				{getNodeById(settingNodeID!)?.data.outputs && (
					<div className='rounded border border-1 p-3 border-blue-500 mt-3'>
						<h3 className='text-lg font-bold pb-2'>输出项：</h3>
						<div className='flex flex-col divide-y'>
							{getNodeById(settingNodeID!)?.data.outputs!.map(
								(output, index) => (
									<div className='relative p-2' key={index}>
										<TextData
											io='input'
